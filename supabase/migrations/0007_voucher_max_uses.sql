-- Teil-B-Härtung: Gutschein-Nutzungslimit serverseitig erzwingen.
-- Ersetzt validate_order (aus 0005): der Gutschein wird jetzt atomar geprüft UND hochgezählt.
-- Ein aufgebrauchter Gutschein (uses >= max_uses) wird wie ein ungültiger behandelt → stiller
-- Voll-Preis, keine Ablehnung. max_uses <= 0 gilt als unbegrenzt.
...
begin
  ...
  if new.voucher_code is not null then
    -- Atomar prüfen + einlösen: der Sub-Select sperrt genau EINE Zeile (for update; code ist
    -- nicht unique → limit 1 wie zuvor), damit zwei gleichzeitige Bestellungen das Limit nicht
    -- überschreiten. Rollback (z. B. späterer Slot-Fehler) macht die Zählung automatisch rückgängig.
     ...
     where id = (
       select id from public.vouchers
        where code = new.voucher_code
          ...
          and (max_uses <= 0 or uses < max_uses) -- max_uses <= 0 = unbegrenzt
        limit 1
        for update
     )
     returning * into v;
    if found then
      ...
      new.voucher_code := null; -- ungültig/abgelaufen/aufgebraucht → ignorieren, kein Reject
    end if;
  end if;
  ...
  -- ── Abhol-Slot prüfen (raise → Insert scheitert, Gutschein-Zählung wird zurückgerollt) ──
