...
begin
  ...
  if not found then
    raise exception 'Konfiguration fehlt'; -- fail closed statt Slot-Checks stillschweigend zu überspringen
  end if;
  ...
  if new.service_mode not in ('dinein', 'takeaway') then
    raise exception 'Ungültiger Service-Modus'; -- erschöpfend: sonst umgeht ein Fremdwert die Service-Gate
  end if;
