...
| P3 | **Idee: Zutaten-Kategorie „Sonstiges"** (z. B. Schoko) — neue `category` ergänzen (vermutlich Code-Konstante). Passt mit Dessert-/Süß-Pizzen. | offen (Ideenstatus) | — |
| P3 | **Idee: „Angemeldet bleiben" beim Login** — Checkbox auf Login-Seite. ❗Nuance: supabase-js persistiert Session per Default (`localStorage`) → Nutzer bleibt eh eingeloggt; echte Option steuert eher Gegenteil (ohne Haken → `sessionStorage`, Logout bei Browser-Schließen). Storage bei `createClient` fixiert → Runtime-Umschalten kniffliger. Eigenes Design. | offen (Ideenstatus) | Teil-B1 |
| P2 | **Idee: versteckter Gutschein → verstecktes Warenkorb-Item mit Menge** — Code (z. B. `weed420`) schaltet ein sonst unsichtbares, bestellbares Item frei, das mit **Anzahl** in den Warenkorb kommt. ❓ Klären: Preis, Sichtbarkeit in Admin-Bestellungen, Cart-Modell muss **Mengen** lernen (kennt aktuell nur Pizzen ohne Menge). Eigenes Design. | offen (Ideenstatus) | — |
